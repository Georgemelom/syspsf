...
public class ServletListarPaciente extends HttpServlet {
	...
	private Paciente criarPaciente(HttpServletRequest request)  {
		...
		if (AuxilioServlets.naoEhNuloOuVazio(request.getParameter("nome")))
			pacienteNovo.setNome(request.getParameter("nome"));
		
		if (AuxilioServlets.naoEhNuloOuVazio(request.getParameter(
				"dataNascimento"))){
			try {
				SimpleDateFormat formato = new SimpleDateFormat("dd/MM/yyyy");
				pacienteNovo.setDataNascimento(formato.parse(request.getParameter(
						"dataNascimento")));
		...
		if (AuxilioServlets.naoEhNuloOuVazio(request.getParameter("sexo")))
			pacienteNovo.setSexo(Integer.parseInt(request.getParameter("sexo")));
