...
public class ServletIncluirPaciente extends HttpServlet {
	...
	protected void tratarAcao(HttpServletRequest request,
		...
		if (acao.equals("adicionarDocumento")) {
			...
			if (!erros.isEmpty()) {
				...
				try {
					request.setAttribute("pessoa", NegocioPaciente
							.incluir(pacienteNovo));

				} catch (ExistePessoaException e) {
					e.printStackTrace();
					erros.add(e.getMessage());
				}
				mensagens.add("Paciente incluída com sucesso!");
			}
	...
	private Paciente criarPaciente(HttpServletRequest request) {
		...
		Paciente pessoaNova = new Paciente();
		pessoaNova.setNome(request.getParameter("nome"));

		try {
			SimpleDateFormat formato = new SimpleDateFormat("dd/MM/yyyy");
			pessoaNova.setDataNascimento(formato.parse(request
					.getParameter("dataNascimento")));
		} catch (ParseException e) {
			e.printStackTrace();
			erros
					.add("Formato da data de nascimento inválido. Use DD/MM/AAAA.");
		}

		pessoaNova.setSexo(Integer.parseInt(request.getParameter("sexo")));

		// TODO AINDA FALTA OS OUTROS CAMPOS. VOU FAZER A PARTE DE PERSISTÊNCIA
		// AGORA.
